...
def fill_sar_properties(sar_ext, href):
    ...
    """
    # Read meta file
    with fsspec.open(href, requester_pays=True) as file:
...
def fill_sat_properties(sat_ext, href):
    ...
    sat_ext.relative_orbit = int(root.findall(".//safe:relativeOrbitNumber")[0].text)
...
def fill_proj_properties(proj_ext, meta_links, product_meta):
    ...
    with fsspec.open(links[0][1].href, requester_pays=True) as file:
        root = XmlElement(etree.fromstring(file.read()))
