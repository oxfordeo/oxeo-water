...
class MetadataLinks:
    def __init__(
        ...
        self.fs = fsspec.filesystem("s3", requester_pays=True)
        self.granule_href = granule_href
        self.href = os.path.join(granule_href, "manifest.safe")
        with self.fs.open(self.href) as file:
            root = XmlElement(etree.fromstring(file.read()))
    ...
    def annotation_hrefs(self) -> List[str]:
        ...
        return [
            "s3://" + x for x in self.fs.ls(path=annotation_path) if x.endswith("xml")
        ]
    ...
    def calibration_hrefs(self) -> List[str]:
        ...
        return [
            ...
            for x in self.fs.ls(path=calibration_path)
